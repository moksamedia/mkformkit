...
var MKFormAssistant = function () {
  ...
  function MKFormAssistant(formDescription, component, options) {
    var _this = this;
    ...
    if (_lodash2.default.has(options, 'handleSubmit')) {
    ...
    if (_lodash2.default.has(options, 'otherState')) {
    ...
    this.formProps = {};
    this.callbacks = {};
    var formState = {}; // we don't maintain the state here once it's built
    this.defaultValues = {};
    ...
    _lodash2.default.forIn(formDescription, function (formItemValues, formItemName) {
      ...
      formState[formItemName].value = _lodash2.default.has(formItemValues, 'value') ? formItemValues.value : formItemValues.default;
      formState[formItemName].visible = _lodash2.default.has(formItemValues, 'visible') ? formItemValues.visible : true;
    ...
    _lodash2.default.forIn(otherState, function (obj, key) {
      if (_lodash2.default.has(formState, key)) {
  ...
  MKFormAssistant.prototype.formItemIsValid = function formItemIsValid(formItemState) {
    return formItemState.validationState == null || formItemState.validationState == 'success';
  };

  MKFormAssistant.prototype.formItemFulfillsRequired = function formItemFulfillsRequired(formItemState, formItemName) {
  ...
  MKFormAssistant.prototype.handleSubmitInternal = function handleSubmitInternal(e) {
    ...
    var invalidFormItemNames = [];

    // CHECK VALIDATION
    _lodash2.default.forIn(this.formDescription, function (value, key) {
      // TODO: need to finish validation and check required state
      //console.log(key +": " + JSON.stringify(value));

      var currentState = _this2.component.state;

      if (!_this2.formItemIsValid(currentState[key])) {
        invalidFormItemNames += [key];
      }

      if (_this2.formProps[key].required && !_this2.formItemFulfillsRequired(currentState[key], key)) {
        invalidFormItemNames += [key];
      }
    });
  ...
  MKFormAssistant.prototype.getValuesFromFormState = function getValuesFromFormState() {
    ...
    _lodash2.default.forIn(this.component.state, function (formItemAttributes, formItemName) {
